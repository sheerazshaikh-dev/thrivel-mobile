...
import { COLORS, Muted, Panel, Screen } from '@/components/ui';
...
export default function Coach() {
  ...
  useEffect(() => {
    if (advisor?.messages?.length || sending) setTimeout(() => scroll.current?.scrollToEnd({ animated: true }), 90);
  }, [advisor?.messages?.length, sending]);
  ...
  return <Screen><SafeAreaView style={s.fill} edges={['top']}>
    <KeyboardAvoidingView style={s.fill} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={0}>
      ...
      {!hasConversation ? <View style={s.emptyWrap}>
        <View style={s.emptyInner}>
          <LinearGradient colors={colors} style={s.emptyBot}><Ionicons name="chatbubble-ellipses-outline" size={25} color="#fff"/></LinearGradient>
          <Text style={s.emptyTitle}>How can I help with your health plan?</Text>
          <Text style={s.emptyCopy}>Ask about meals, workouts, sleep, recovery, your purchased products, progress, or your reviewer-published plan. If another catalog product fits a new goal, the coach can point you to it for review before checkout. Medication changes and research-product administration remain outside the coach.</Text>
          <Composer value={message} setValue={setMessage} send={send} sending={sending} inputRef={input} colors={colors} centered/>
          <Text style={s.limit}>{advisor.rateLimit ? `${advisor.rateLimit.remaining} messages remaining this hour · ` : ''}Health and {branding.brandName} topics only.</Text>
        </View>
      </View> : <>
        ...
        <View style={s.composerDock}>
          <View style={s.composerMax}>
            ...
            <Text style={s.footerNote}>{advisor.rateLimit ? `${advisor.rateLimit.remaining} messages remaining this hour. ` : ''}AI Health Coach can explain and support the published plan but cannot change medication or dosage.</Text>
          </View>
        </View>
      </>}
...
function Composer({ value, setValue, send, sending, inputRef, colors, centered = false }: { value: string; setValue: (v: string) => void; send: () => void; sending: boolean; inputRef: RefObject<TextInput | null>; colors: readonly [string,string,string,string]; centered?: boolean }) {
  return <View style={[s.composer, centered && s.composerCentered]}>
...
const s = StyleSheet.create({
  ...
  emptyWrap: { flex: 1, justifyContent: 'center', paddingHorizontal: 16, paddingVertical: 28 },
  emptyInner: { width: '100%', maxWidth: 768, alignSelf: 'center', transform: [{ translateY: -28 }] },
  ...
  composer: { minHeight: 64, flexDirection: 'row', alignItems: 'flex-end', gap: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,.15)', backgroundColor: 'rgba(255,255,255,.055)', borderRadius: 28, padding: 8 },
  composerCentered: { padding: 10 },
  input: { flex: 1, minHeight: 46, maxHeight: 176, color: '#fff', paddingHorizontal: 12, paddingVertical: 12, fontSize: 14, lineHeight: 20, textAlignVertical: 'top' },
  sendHit: { alignSelf: 'flex-end' },
  send: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  sendDisabled: { opacity: .42 },
  limit: { color: 'rgba(255,255,255,.35)', fontSize: 10.5, textAlign: 'center', marginTop: 11 },
